...
Protokół to zestaw reguł, które określają, w jaki sposób urządzenia komunikują się przez sieć. Definiują format wiadomości, kolejność ich wysłania oraz sposób reagowania ba błędy. Bez protokołów urządzenia różnych producentów nie mogłyby się ze sobą porozumieć. 
...
- zagrożenia: AQL injection ( wstrzyknięcie kodu SQL przez parametr URL), XSS (wstrzyknięcie JavaScript  w stronę), path traversal (../../etc/passwd). Należy także pamiętać, że dane w tym protokole nie są szyfrowane, więc są wrażliwe na podsłuch. W logach widać każde żądanie - metodę, URL oraz kod odpowiedzi. 
...
- zagrożenia: podstawowym zagrożeniem dla tego protokołu jest atak brute force, np atakujący próbuje haseł by dostać się na konto root. W logach `auth.log` widać `Failled password for root from IP`. Niebezpieczne jest gdy po wielu takich nieudanych logowaniach z danego IP następuje `Accepted password for root` - w takim przypadku jest prawdopodobne, że konto zostało skompromitowane. 
...
- jak działa: działa na zasadzie 4 kroków:
	- Discover - klient wysyła wiadomość: "Jestem nowy, czy ktoś tu rozdanie adresy IP?",
	- Offer - serwer odpowiada: "Cześć jestem mam dla Ciebie adres 192.186.1.25",
...
- port: 123,
- transport: UDP,
- co robi: synchronizuje zegary miedzy urządzeniami. To jest krytyczne dla logów, bo bez poprawnego czasu skorelowanie zdarzeń jest utrudnione, 
...
- jak działa "TCP handshake - tree-way handshake"):
...
- jak działa: routery czytają adres docelowy i decydują, którędy wysłać pakiet dalej. 
...
- jak działa: ping, czyli wysłanie małych pakietów, by sprawdzić czy serwer odpowie, jeśli nie odpowiada, oznacza to, że brak połączenia. 
- zagrożenia: ICMP tunneling - ukrywanie ruchu C2 w pakietach ICMP (pingi, które niosą dane), Ping flood - zalewnie ICMP echo request.
...
- zagrożenia: ARP spoofing/poisoning - atakujący wysyła fałszywy ARP Reply ("jak jestem bramą 192.168.1.1") i przechwytuje cały ruch sieciowy ofiary. Tablice ARP można podejrzeć za pomocą polecenia `ip neigh` lub `arp -a`. 
