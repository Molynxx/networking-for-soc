...
- Należy losować query ID oraz porty źródłowe - zabezpieczenie, które znacznie utrudnia poisoning. Na czym polega:
	...
	Należy jednak zaznaczyć ze nowoczesne resolvery losują Query ID dla każdego zapytania, losują port źródłowy z szerokiego zakresu, używają losowości kryptograficznej.  
...
- ten atak może się udać tylko w przypadkach gdy:
	- resolver jest stary - nie losuje quwey ID i portów, 
...
- próby AXFR - polecenie, próby pobrania całej strefy, czyli wszystkich informacji. 
...
Atak ten różni się od DNS amplification kierunkiem ruchu (NXDOMAIN - ruch od resolvera, amplification - z resolvera do ofiary), celem (NXDOMAIN - zmęczyć reslover, amplification - zalać ofiarę) oraz IP ofiary (NXDOMAIN - prawdziwe, amplification - fałszywe). 
