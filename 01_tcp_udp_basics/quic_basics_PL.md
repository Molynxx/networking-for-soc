...
QUIC to nowy protokół transportowy na UDP, który łączy szybkość UDP, bezpieczeństwa TLS 1.3, niezawodność TCP, ale bez jego wad. Został stworzony przez Google, aby przyspieszyć internet oraz rozwiązać problemy, które istnieją w połączeniu TCP, takie jak:
- wolny handshake (2-3 RTT z TLS) - RTT (Round Trip TIme) to czas od wysłania pakietu do otrzymania odpowiedzi, np.: SYN -> SYN-ACK wynosi 1 RTT, jeśli dołożyć do tego TLS czas czas zanim dane w ogóle ruszą wydłuża się do 2-3 RTT, podczas gdy RTT dla QUIC wynosi 0-1. 
- head-of-line blocking - w TCP jeden zgubiony pakiet blokuje cały strumień, 
- zrywa połączenie po zmianie sieci np: WiFi -> LTE - QUIC ma Connection ID, czyli identyfikator połączenia, który nie zależy od IP i portu. Dzięki temu zmiana sieci nie wpływa na połączenie. Ma to jednak jedną wadę dla SOC - jest trudniej namierzyć atakującego. 
...
- cel: DDoS, 
- jak działa: atakujący wysyła duże ilości pakietów na serwer ofiary, ponieważ QUIC działa na UDP a UDP jest bezstanowy, serwer nie wiem czy te pakiety przychodzą od zwykłego użytkownika czy od atakującego. Każdy pakiet QUIC wymaga od serwera: prasowania, sprawdzania, odpowiadania, więc przy dużej ilości pakietów serwer zużywa CPU, pamięć oraz łącze, 
- jak wykryć: 
	- widoczny wzrost ruchu UDP na porcie 443, 
	- duża liczba pakietów z jednego IP, 
	- wzrost zużycia CPU na serwerze, co oznacza, że serwer bardzo nad czymś pracuje, 
	- spadek responsywności usługi.
- jak się bronić: 
	- rate limitig na UDP 443 - ograniczy ilość połączeń z jednego IP, 
	- filtrowanie pakietów QUIC na firewall - nowoczesne firewalle uczą się śledzić połączenia QUIC, dzięki czemu mogą sprawdzać czy ruch jest normalny (np. czy pochodzi od prawdziwego klienta). Dodatkowo mogą blokować podejrzane wzorce. 
	- usługi anty-DDos (np. Cloudflare, Akamai) - to usługi lub urządzenia stojące przed serwerem, które mogą:
		- absorbować ruch - wytrzymują ogromne ataki, które zabiłyby by przeciętny serwer, 
...
- jak się bronić:
	- ustawienie konfiguracji serwera tak żeby akceptował 0-RTT tylko da żądań idempotentnych (np. GET) - czyli dla operacji, które można bezpiecznie powtarzać. Nie dla operacji zmieniających stan jak np. POST, PUT, DELETE, 
	- ustawienie w konfiguracji serwera jednorazowych tokenów - serwer wymaga unikalnego tokeny, który może być użyty tylko raz, 
	- ustawienie na serwerze którego czasu ważności 0-RTT - czyli 0 RTT jest akceptowane tylko przez kilka sekund od pierwszego żądania, 
...
- jak wykryć: 
	...
	- dwa rozne IP używają tego samego Connection ID jednocześnie:
...
- jak się bronić:
	- konfiguracja serwer:
		- serwer ma generować losowe, długie Cnnection ID, 
...
- jak wykryć:
	...
	- odpowiedz jest większa niż zapytanie, 
	- IP źródłowe, które nie wysyłało zapytań, 
- jak się bronić: 
	- serwery QUIC mają wbudowany mechanizm obrony `anti-amolification limit (3x)` - oznacza to, że serwer nie może wysłać odpowiedzi niż 3x większych niż zapytanie, które dostał. Ta opcja domyślnie jest włączona, nie należy jej wyłączać, 
	...
	- `rate limiting` - konfiguracja na firewallu, pozwała ustalić limity połączeń serwera z jednym IP w określonym oknie czasowym, 
...
### Jak monitorować UQUIC w praktyce
...
- analiza wolumenu i częstotliwości, gdzie zobaczyć można:
	...
	- jak często, 
	- do kogo,   
	To wystarczy żeby wykryć beaconing, exfiltrację, DDoS. 
