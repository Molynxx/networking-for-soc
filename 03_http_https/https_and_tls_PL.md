...
HTTPS zapewnia więc nam poufność, integralność (nikt nie zmieni danych po drodze) oraz uwierzytelnienie (daje pewność, że połączona z prawdziwym serwerem). HTTPS nie tyle jest osobnym protokołem, co połączeniem HTTP z szyfrowaniem w warstwie prezentacji. Można to zobrazować na podstawie warstw OSI:
- HTTP: 
...
- HTTPS:
	...
	- warstwa transportowa: TCP, 
	- warstwa sieciowa: IP. 
Szyfrowanie pozwala ukryć wszystkie informacje przed osobami trzecimi, treść danych jest w tym przypadku niejawna. SOC zobaczyć może jedynie:
...
- Krok 1 - ClientHello:
	- klient wysyła wersje TLS (np. 1.1, 1.2), 
	- listę cipher suities (czyli algorytmy szyfrowania), 
...
- Krok 3 - Weryfikacja certyfikatu - klient sprawdza:
	...
	Jeśli cokolwiek się nie zgadza - klient przerwa połączenie. 
...
- Symetryczne:
	- jeden klucz, 
	- jest szybkie - przy większych ilościach danych do bardzo przyspiesza komunikację, 
...
- Certyfikat to dokument cyfrowy, który potwierdza, że dana domena należy do danej firmy. Certyfikat zawiera: 
	...
	- podpis CA.
- Łańcuch zaufania:    
	`Root CA -> Interemediate CA -> Certyfikat serwera`    
	...
	-  Intermediate CA - pośrednik, upoważniony do wystawienia certyfikatu przez Root CA. Lista Intermediate CA nie jest wbudowana w systemy czy przeglądarki, więc nie wie, który Intermediate jest zaufany. 
	- Certyfikat serwera - podpisany przez Itermediate CA. Ponieważ system/przeglądarka nie wie, który Intermediate jest zaufany, serwer wysyła klientowi swój certyfikat oraz certyfikat Intermediate CA podpisany przez Root CA. Klient, ma wbudowaną listę Root CA więc sprawdza czy certyfikat Intermediate jest podpisany przez zaufany Root CA. Jeśli tak - ufa Intermediate CA i sprawdza czy certyfikat serwera jest podpisany przez ten Intermediate CA i jeśli wszystko się zgadza ufa serwerowi. 

## SNI - Server Name Indication 
SNI to nazwa domeny, na którą klient wysyła ClientHello. Jest to istotne ponieważ jeden serwer może obsługiwać wiele domen i to właśnie SNI mówi serwerowi, z której domeny klient chce zobaczyć certyfikaty. Jest ono widoczne w handshake nawet jeśli dane są szyfrowane. Dzięki temu można sprawdzić z jaką domeną łączy się klient, 
...
- self-signed cery - czyli certyfikat wystawiony przez serwer samemu sobie - często w malware, 
- SNI spoofing - czyli podszywanie się pod domenę.  
Szczegóły tych ataków i sposób ich wykrywania są opisane w w folderze 04_tls. 
